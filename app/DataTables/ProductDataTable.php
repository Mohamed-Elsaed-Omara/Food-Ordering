    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $edit = "<a href='" . route('admin.product.edit', $query->id) . "' class='btn btn-primary' title='edit'><i class='fas fa-edit'></i></a>";
                $delete = "<a href='" . route('admin.product.destroy', $query->id) . "' class='btn btn-danger mx-2 delete-item' title='delete' ><i class='fas fa-trash'></i></a>";
            ...
            ->addColumn('image', function ($query) {
                $image = "<img src='" . asset($query->thumb_image) . "' width='60px'>";
                return $image;
            })
            ->rawColumns(['price', 'offer_price', 'status', 'show_at_home', 'image', 'action'])
            ->setRowId('id');
    }
